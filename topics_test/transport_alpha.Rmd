...
```{r}
Ks <- c(5, 5)
#lda_results <- list("topics" = fit_ldas(x$topics, Ks, alpha_theta=0.1))
lda_results <- list("topics" = fit_lda_alphas(x$topics, Ks[1], alpha_thetas=c(0.01, 10)))
```

```{r}
theta_split <- lda_results$topics$thetas %>%
  split(.$alpha_theta) %>%
  map(~ dcast(.x, alpha_theta + document ~ topic, value.var = "gamma"))
topic_weights <- theta_split %>%
  map(~ .x %>% dplyr::select(-alpha_theta, -document)) %>%
  map(colSums)
```

```{r}
beta_hat <- lda_results$topics$betas %>%
  dcast(alpha_theta + topic ~ term, value.var = "beta") %>%
...
output_data$alpha_theta <- 0
output_data[1:nrow(beta_hat), "alpha_theta"] <- beta_hat$alpha_theta
```

```{r}
beta_mats <- beta_hat %>%
  split(.$alpha_theta) %>%
  map( ~ .x %>% dplyr::select(-topic, -alpha_theta)) %>%
  map(~ as.matrix(.x))
```
...
rownames(sink_res) <- paste(0.01, 1:nrow(sink_res), sep = "_")
colnames(sink_res) <- paste(10, 1:ncol(sink_res), sep = "_")
...
ggplot() +
  ...
  geom_point(
    ...
    aes(x = `Dim 1`, y = `Dim 2`, col = as.factor(alpha_theta), size = topic_weight)
  ) +
  geom_point(
  ...
  labs(x = "D1", y = "D2", col = expression(alpha)) +
  scale_size(range = c(0.05, 4), guide = FALSE) +
  scale_alpha(range = c(0.05, 1), guide = FALSE) +
  scale_color_brewer(palette = "Set2") +
  coord_fixed()
ggsave(sprintf("transport-alphas-%s.png", paste0(Ks, collapse="_")))
```